		...
		showCalendar:function(options){
			...
			var Calendar = {
						...
						for(var j=1;j<=7;j++) {
							...
							if(i==0) {
								...
								if((i-1)*7+j<=oneWeek) { //从第二行开始设置值
									...
									dayHTML+="<td class='this_month_day"+
										(result==new Date().getDate()?'today':'')+"'>"
									+result+"</td>";
								} else {
				...
				tableStyle:function(){
					...
					$calTable.find("td.select").css({"background-color":settings.weekBgColor,
						"color":settings.thisMonthFontColor,
						"border-radius":"50%"
					});
					$calTable.find("td.today").css({"background-color":settings.todayBgColor,
						"color":settings.thisMonthFontColor,
						"border-radius":"50%"});
					...
					$calTable.find("tr[class!=week_head] td").hover(function(){
						...
						else if($(this).hasClass("this_month_daytoday")){
							$(this).css({"background-color":settings.todayBgColor,
							"color":settings.thisMonthFontColor,
							"border-radius":"50%"})
						}
						else {
